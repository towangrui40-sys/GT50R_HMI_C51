...
#define SPK_TIME_OUT_TIMES 5		   //��������5��
void Task_Timer_Sec(void)  //  250ms����һ��
{
	u16 xdata temp;
	static u16 xdata tick = 0;  //1s��ʱ
...
/*********************״̬��******************************/
void Run_FSM(void)   //FSM״̬�� 
{

	switch(win_str_data.state)
	{
		case FSM_INIT:    //***************״̬����״̬����ʼ��*****************************************************
			win_str_data.state = FSM_READY;   //��ʼ��λREADY״̬
			break;
		case FSM_READY:   //****************״̬����׼��״̬********************************************************	
				/**************��ʾֵ�趨Ϊ����ֵ*******************/
...
//				win_disp_level(MAIN_TIMER,win_str_data.use_level);
				/**************************************************/
				if(win_str_data.start == 1 && win_str_data.door_warning == 0)    //���յ�Start_win  start=1��ʼ�ź�
				{
	...
	case FSM_RUN:/*******************************************״̬��������״̬*********************************/   //���й�����
				...
				if(win_str_data.pause == 1)/********** pause = 1��ͣ�ź� pause = 0����ͣ�ź�******************************/
				{
					RTU_Cmd_Pending(RTU_STOP,MC_STOP);   //send motor stop
					win_str_data.state = FSM_PAUSE;
				}	
				else if(win_str_data.start == 0) //start = 0ֹͣ�ź� start = 1 �����ź�
				{
					RTU_Cmd_Pending(RTU_STOP,MC_STOP);   //send motor stop
					win_str_data.state = FSM_STOP;
				}				
				else if(win_str_data.door_warning == 1)/********** ���й����п���**********/
				{
					win_str_data.start = 0;
			...
			break;
		case FSM_PAUSE: /**********************************״̬������ͣ״̬********************************/
				if((win_str_data.start == 1) && (win_str_data.pause == 0))     //�����а�ť,�ָ�����
				{
					win_str_data.state = FSM_START;
				}
				if((win_str_data.start == 0) && (win_str_data.pause == 0)) //start = 0ֹͣ�ź� start = 1 �����ź�
				{
					win_str_data.state = FSM_STOP;
				}
			break;
...
{
	u16 vol_temp;
	vol_temp = win_level_trans_volt(win_str_data.use_level);   //��ȡ�����ѹֵ
	if(str_sys.set_vol != vol_temp)	   //��ȡ����λ����ѹֵ���������õ�ѹֵ
	{
		str_sys.set_vol = vol_temp;
